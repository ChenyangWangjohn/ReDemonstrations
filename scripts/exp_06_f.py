...
Description: Finetuned model (corrupted) with random ctx_a to test performance on mismatched context
Model: M_finetuned (Qwen3-1.7B finetuned on MathInstruct)
Context: Few-Shot (Random ctx_a)
"""
...
def run_experiment_f(
    ...
    """Run Experiment F: M_finetuned Few-Shot with Random ctx_a"""
    print(f"\n{'='*70}")
    print(f"Experiment 06: M_finetuned Few-Shot with Random ctx_a")
    print(f"{'='*70}\n")
    ...
    with torch.no_grad():
        for idx, example in enumerate(tqdm(test_data, desc="Experiment 06")):
            # Build prompt (few-shot with random ctx_a)
            prompt = create_few_shot_prompt(few_shot_examples, example, ctx_a_type="random")
    ...
    if output_file:
        ...
        summary = {
            "experiment": "06_F",
            "description": "M_finetuned Few-Shot with Random ctx_a",
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 06: M_finetuned Few-Shot with Random ctx_a")
    parser.add_argument("--finetuned_model_path", type=str, required=True, help="Path to finetuned model")
    parser.add_argument("--random_data_path", type=str, required=True, help="Path to random dataset JSON")
